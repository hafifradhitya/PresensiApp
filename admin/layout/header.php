...
// Ambil data ketidakhadiran yang masih PENDING untuk notifikasi
$notif_ketidakhadiran = mysqli_query(
    $connection,
    "SELECT k.*, p.nama 
...
$jumlah_pending = mysqli_num_rows($notif_ketidakhadiran);

// Simpan hasil ke array supaya bisa dipakai di HTML
$notif_items = [];
while ($row = mysqli_fetch_assoc($notif_ketidakhadiran)) {
    $notif_items[] = $row;
}
...
<html lang="en">
  <head>
    ...
    <style>
      body.theme-dark .hide-theme-dark { display: none !important; }
      body.theme-light .hide-theme-light { display: none !important; }
    </style>
  </head>
<body class="theme-light">
      ...
      <header class="navbar navbar-expand-md navbar-light d-print-none">
        <div class="container-xl">
          ...
          <div class="navbar-nav flex-row order-md-last">
            <!-- MENU DARK/LIGHT MODE & NOTIFICATION -->
             <a href="#"
                class="nav-link px-0 hide-theme-dark theme-toggle"
                data-theme="dark"
              ...
              <!-- Tombol Light (ikon matahari) -->
              <a href="#"
                class="nav-link px-0 hide-theme-light theme-toggle"
                data-theme="light"
  ...
  <a href="#" class="nav-link px-0" data-bs-toggle="dropdown" tabindex="-1" aria-label="Show notifications">
    ...
    <?php if ($jumlah_pending > 0) : ?>
      <span class="badge bg-red"><?= $jumlah_pending ?></span>
    <?php endif; ?>
  </a>
  ...
  <div class="dropdown-menu dropdown-menu-arrow dropdown-menu-end dropdown-menu-card">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Pengajuan Ketidakhadiran Pending</h3>
      </div>
            ...
            <div class="row align-items-center">
              <div class="col text-truncate">
                <div class="text-muted">
                  Tidak ada pengajuan ketidakhadiran yang pending.
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="list-group-item">
              <div class="row align-items-center">
                ...
                <div class="col text-truncate">
                  ...
                  <a href="<?= base_url('admin/data_ketidakhadiran/detail.php?id='.$item['id']) ?>"
                    class="text-body d-block">
                    <?= htmlspecialchars($item['nama']) ?> - <?= htmlspecialchars($item['keterangan']) ?>
                  </a>
                  <!-- Tanggal + Deskripsi singkat -->
                  <div class="d-block text-muted text-truncate mt-n1">
                    <?= date('d M Y', strtotime($item['tanggal'])) ?> -
                    <?= htmlspecialchars(substr($item['deskripsi'], 0, 50)) ?><?= strlen($item['deskripsi']) > 50 ? '...' : '' ?>
                  </div>
                </div>
                <div class="col-auto">
                  <a href="<?= base_url('admin/data_ketidakhadiran/detail.php?id='.$item['id']) ?>"
                    class="list-group-item-actions" title="Lihat Detail">
                    <!-- Icon bintang / detail -->
        ...
        <div class="page-header d-print-none">
          <div class="container-xl">
            <div class="row g-2 align-items-center">
              ...
              <!-- Page title actions -->
              <!-- Disini tadinya ada button buat create new report -->
            </div>
          </div>
        </div>
...
<script>
  (function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
      // Ambil tema dari localStorage / prefers-color-scheme
